...
class AddTransactionScreen(ttk.Frame):
    ...
    def __init__(self, master, transaction_type: TransactionType):
        ...
        self.transaction_context.set_strategy(
            DebitStrategy()
    ...
    def _setup_widgets(self):
        ...
        self.bank_account_combobox.grid(
            row=1, column=0, sticky=tk.EW, columnspan=12, pady=10
        )
    ...
    def on_submit(self):
        try:
            ...
            bank_account = next(
                (acct for acct in self._bank_accounts if acct.iban == selected_iban)
            )
