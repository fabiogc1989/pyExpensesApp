...
class TransactionViewRepository(BaseReadRepository[TransactionViewSchema]):
    @override
    def get_all(self) -> list[TransactionViewSchema] | Iterator[TransactionViewSchema]:
        try:
            with SessionLocal() as session:
                entities = session.query(TransactionView).all()
                return [
                    TransactionViewSchema.model_validate(entity) for entity in entities
                ]
    ...
    def get(self, id: int) -> TransactionViewSchema:
        try:
            ...
            raise RepositoryException(
                f'Error fetching transaction view with ID {id}: {str(e)}'
            )

    def search_transactions(
        self, search_model: TransactionSearch
    ) -> list[TransactionViewSchema] | Iterator[TransactionViewSchema]:
        try:
            with SessionLocal() as session:
                ...
                if search_model.iban is not None:
                    query = query.filter(
                        TransactionView.bank_account.has(iban=search_model.iban)
                    )
                if search_model.min_amount is not None:
                    query = query.filter(
                        TransactionView.amount >= search_model.min_amount
                    )
                if search_model.max_amount is not None:
                    query = query.filter(
                        TransactionView.amount <= search_model.max_amount
                    )
                ...
                if (
                    search_model.type is not None
                    and search_model.type != search_model.type.ALL
                ):
                    query = query.filter(TransactionView.type == search_model.type.name)
                if search_model.description is not None:
                    query = query.filter(
                        TransactionView.description.ilike(f'%{search_model.description}%')
                    )
